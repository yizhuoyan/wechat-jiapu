...
var uniqueArray = function(arr) {
  ...
  for (var i = 0;
    (item = arr[i]) != null; i++) {
    temp = item.replace(/[ol](?=s|b)/, 'x').replace(/&[ol]/, ''); //对特殊语法标识相互包含的行为去重
...
//分词解析
const getSelectors = function(str) {
...
//简化选择器
const selector2id = function(selector, sex) {
  var result = [];
  var hash = {};
  var sex2 = -1;
  if (sex < 0) { //如果自己的性别不确定
...
const relationship = function(options) {
